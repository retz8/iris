...
## Optimization: line_range null

### Problem Observed

**Variable declarations:**
```javascript
const MAX_RETRY = 5;  // line 10
```

**Before optimization:**
```json
// AST
{"type": "variable_declaration", "line_range": [10, 10], "name": "MAX_RETRY"}

// Stage 1
"Mark for reading (vague or not)" → ranges_to_read: [10, 10]

// Tool reads
"const MAX_RETRY = 5;" ← Redundant! Already in AST
```

**Issue:** Single-line declarations have no hidden implementation. Reading provides no additional context.

### Solution

**Mark single-line nodes with `line_range: null`:**
...
```json
{
  ...
  "line_range": null,  // ← Signals "no impl to read"
  "name": "MAX_RETRY"
}
```
...
1. **Token savings:** ~10-20% reduction in tool reads
2. **Clear semantics:** `null` = "nothing to read"
3. **Faster Stage 1:** Immediate skip, no decision needed

### Examples

**Skip these (line_range: null):**
```javascript
const MAX_RETRY = 5;                              // Single-line constant
import * as THREE from "three";                   // Single-line import
const add = (a, b) => a + b;                      // Single-line arrow function
const { x, y } = config;                          // Single-line destructuring
const settings = { timeout: 5000, retry: 3 };     // Single-line object
```

**Consider these (line_range: [start, end]):**
```javascript
function process(data) {     // Multi-line function
  return data.map(x => x);
}

const config = {             // Multi-line object
  timeout: 5000,
  retry: 3,
  debug: true
};
