---
name: setup-jioh-agent-workflow
description: "Sets up the multi-session agent workflow coordination structure under docs/tasks/. Creates the docs/tasks/ folder, README.md (workflow rules), and UPDATES.md (handoff log). Use when starting a new round of parallel Claude Code track work."
...
Set up the coordination layer for a multi-session Claude Code workflow under `docs/tasks/`.

## Steps

1. Check if `docs/tasks/` already exists. If it does, report what's already there and stop — do not overwrite.

2. Create the directory: `docs/tasks/`

3. Create `docs/tasks/README.md` with the following content exactly:
...
4. Create `docs/tasks/UPDATES.md` as an empty file with just a header:
